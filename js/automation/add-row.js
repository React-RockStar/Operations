$(function() {
	$(document).on('click', 'a.add-carrier-row', function(e){
		...
		$('html, body').animate({
			scrollTop: $("#add-additional-carrier-container .carrier-row:last-child").offset().top
		}, 1000);
	...
	$(document).on('change', '#carrierSubmitted', function() {
		var value = $('#carrierSubmitted option:selected').text();
		var othercarrierSubmitted = $('#otherCarrier');
